    ...
    return (
      <div className="w-full h-full flex items-center justify-center py-4 laptop:py-6">
        ...
        <div className="grid grid-cols-2 gap-2 mobile-M:gap-2.5 tablet:grid-cols-4 tablet:gap-3 laptop:grid-cols-1 laptop:gap-4 laptop-L:gap-5 w-full laptop:h-full auto-rows-fr">
          <StatCardFindJobs title={t.jobs.findJobs.stats.totalJobs} value={stats.totalJobs ?? 0} variant="variant1" className="min-w-0" />
          <StatCardFindJobs title={t.jobs.findJobs.stats.ratings} value={stats.ratings ?? 0} variant="variant2" className="min-w-0" />
          <StatCardFindJobs title={t.jobs.findJobs.stats.completed} value={stats.completed ?? 0} variant="variant3" className="min-w-0" />
          <StatCardFindJobs title={t.jobs.findJobs.stats.posted} value={stats.posts ?? 0} variant="variant4" className="min-w-0" />
        </div>
      </div>
    );
  }
