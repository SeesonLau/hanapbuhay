      ...
      {displayOptions.map((opt) => {
        ...
        return (
          <button
            key={opt.value}
            type="button"
            className={`group relative w-full rounded-lg border transition-all duration-300 ease-out hover:scale-[1.03] hover:shadow-md ${baseHeightClass} ${rowSpanClass} ${isSelected && isSkilled ? 'col-span-2 tablet:col-span-2' : ''} ${isSelected && opt.value === JobType.SERVICE ? 'mobile-S:col-start-2 mobile-M:col-start-2 mobile-L:col-start-2 tablet:col-start-3 laptop:col-start-3 laptop-L:col-start-3' : ''} ${opt.value === JobType.SKILLED ? 'overflow-visible' : 'overflow-hidden'}`}
            style={{
              backgroundColor: isSelected && isSkilled ? theme.colors.pastelBgLight : theme.colors.surface,
          ...
          >
            ...
            {!isSelected && (
              <>
                ...
                <div
                  ...
                  style={{ 
                    backgroundColor: theme.colors.pastelBgLight,
            ...
            <div
              className={`${opt.value === JobType.SKILLED ? 'relative' : 'absolute inset-0'} p-2 mobile-L:p-2 tablet:p-3 transition-all duration-300 ease-out ${opt.value === JobType.SKILLED ? '' : 'overflow-y-auto'} ${(isCreative || isSkilled) ? 'scrollbar-hide' : ''} ${isSelected ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}
              style={{ backgroundColor: theme.colors.pastelBgLight }}
